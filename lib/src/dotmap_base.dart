...
class DotMap {
  ...
  static PathValue pathValue(String path, dynamic value) => PathValue(path: path, value: value);
  ...
  /// ```
  static T? get<T>(map, String path, {T Function(dynamic)? converter}) {
  ...
  /// ```
  static merge(List<Map<String, dynamic>> maps) {
  ...
  static create(
    ...
    for (var pathValue in pathValues) {
      list.add(DotMap._create(pathValue));
    }
    return DotMap.merge(list);
  }
}
